...
for i = 1:1000
    for n = 1:10
        ...
        eqn1 = diff(Ireal) == 0.3*(1-Ireal-Rreal)*p-0.1*Ireal;
        eqn2 = diff(Rreal) == 0.1*Ireal;
        eqns = [eqn1 eqn2];
...
%Ireal = 3*exp(-(3*x)/10)*(exp((3*x)/10)/2 - 9/20) - exp(-x/10)*((3*exp(x/10))/2 - 29/20)
%Rreal = exp(-x/10)*((3*exp(x/10))/2 - 29/20) - exp(-(3*x)/10)*(exp((3*x)/10)/2 - 9/20)
